...
const Theories = () => {
  ...
  const handleUpdate = async (data, name) => {
    switch (name) {
      case "subject":
        const resSubject = await updateSubject(
          dataSelected.subjectId,
          {
            classIds: [1, 2, 3, 4, 5],
            subject_name: data,
            methods: [
      ...
      case "chapter":
        const resChapter = await updateChapter(
          dataSelected.chapterId,
          {
            chapter_name: data,
            classId: dataSelected.class,
            subject: dataSelected.subject,
      ...
      case "theory":
        const resTheory = await updateTheory(
          dataSelected.theoryId,
          {
            theory_name: data,
            classId: dataSelected.class,
            subject: dataSelected.subject,
  ...
  const handleDelete = async (name) => {
    switch (name) {
      case "subject":
        const resSubject = await deleteSubject(
          dataSelected.subjectId,
          user.token
        );
        if (resSubject.status !== 200) {
      ...
      case "chapter":
        const resChapter = await deleteChapter(
          dataSelected.chapterId,
          user.token
        );
        if (resChapter.status !== 200) {
      ...
      case "theory":
        const resTheory = await deleteTheory(dataSelected.theoryId, user.token);
        if (resTheory.status !== 200) {
          messageApi.open({
            type: "error",
            content: "Có lỗi !!!",
  ...
  const handleDeleteLesson = async (lessonId) => {
    const res = await deleteLesson(dataSelected.theoryId, lessonId, user.token);
    if (res.status !== 200) {
      messageApi.open({
        type: "error",
        content: "Có lỗi !!!",
